...
contract Pay {
    ...
    function pickCooperator(address payable cooperatorAddress)
        public
        onlyCreator
    {
        Project storage project = activeProject[0];
        ...
        require(
            cooperatorAddress != project._creator,
            "You cannot set yourself as cooperator."
        );
        project._cooperator = cooperatorAddress;
        project._hascooperator = true;
    }

    function acceptProject() public onlyCooperator {
        Project storage project = activeProject[0];
    ...
    function declineProject() public onlyCooperator {
        ...
        require(
            project._hascooperator,
            "There is no cooperator for this project"
        );
        delete (project._cooperator);
        project._hascooperator = false;
    }

    function fundProject() public payable onlyCreator {
        Project storage project = activeProject[0];
        ...
        onlyCreator
    {
        ...
        require(
            project.projectState == ProjectState.funded,
            "You have not funded this project yet. There are no funds to release."
        );
        require(
            project._value <= address(this).balance,
            "Trying to release more money than the contract has."
        );
        ...
        onlyOwner
    {
        ...
        require(
            project.projectState == ProjectState.funded,
            "You have not funded this project yet. There are no funds to send back."
        );
        require(
            project._value <= address(this).balance,
            "Trying to release more money than the contract has."
